...
public void decreaseRGB(double rAmount, double gAmount, double bAmount)
{
  //create a pixel array
  Pixel[] pixelArray = this.getPixels();
  //create a pixel object
  Pixel p = null;
  //declare count as an int data type and assign a value of 0 to it
  int count = 0;
  //loop through all of the pixels using a for-each loop
  for(Pixel a: pixelArray)
  {
    //get the current pixel
    p = pixelArray[count];
    //decrease the RGB values
    p.setRed((int)(p.getRed()*rAmount));
    p.setGreen((int)(p.getGreen()*gAmount));
    p.setBlue((int)(p.getBlue()*bAmount));
